...
func (s *RedisStore) hold(b Booking) (Booking, error) {
	...
	if err := s.rdb.Set(ctx, sid, key, defaultHoldTTL).Err(); err != nil {
		return Booking{}, err
	}

	fmt.Printf("SESSION(KEY): %s, KEY(VALUE): %s", sid, key)
...
func (s *RedisStore) getSession(ctx context.Context, sessionID string, userID string) (Booking, string, error) {
	...
	if session.UserId != userID {
		return Booking{}, "", fmt.Errorf("unauthorized: session belongs to another user")
	}
